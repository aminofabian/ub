...
public class StockTakeService {
    ...
    private static final DateTimeFormatter NAME_DATE_FORMATTER = DateTimeFormatter.ofPattern("d MMM yyyy");
        ...
        if (itemIds == null || itemIds.isEmpty()) {
            if (InventoryConstants.STOCKTAKE_SESSION_TYPE_EVENING.equals(req.sessionType())) {
                ...
                itemIds = stockTakeSessionRepository
                        ...
                        .filter(l -> InventoryConstants.STOCKTAKE_LINE_CONFIRMED.equals(l.getStatus()))
            ...
            if (itemIds == null || itemIds.isEmpty()) {
                ...
                if (itemIds.isEmpty()) {
                    itemIds = itemRepository.findStockedItemIdsByBusinessId(businessId);
                }
            }
        }

        int sessionNumber = stockTakeSessionRepository.nextSessionNumber(businessId);
    ...
    public Optional<StockTakeSessionResponse> getActiveSession(
            String businessId, String branchId, LocalDate date) {
        return stockTakeSessionRepository
                .findActiveByBusinessIdAndBranchIdAndDateFetchLines(businessId, branchId, date)
                .map(this::buildResponse);
    }

    @Transactional(readOnly = true)
    public Optional<StockTakeSessionResponse> getStaleSession(
            String businessId, String branchId, LocalDate today) {
        ...
        return stockTakeSessionRepository
                .findStaleListByBusinessIdAndBranchIdFetchLines(businessId, branchId, today)
                .stream()
                .findFirst()
                .map(this::buildResponse);
    }
    ...
    public List<StockTakeSessionResponse> listSessions(
            String businessId, String branchId, String status, LocalDate from, LocalDate to) {
    ...
    public StockTakeSessionResponse applySingleCount(
        ...
        StockTakeSession session = loadSessionInProgress(businessId, sessionId);
        StockTakeLine line = session.getLines().stream()
                .filter(l -> l.getId().equals(lineId))
                .findFirst()
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Unknown line id"));
        if (InventoryConstants.STOCKTAKE_LINE_CONFIRMED.equals(line.getStatus())) {
            throw new ResponseStatusException(HttpStatus.CONFLICT,
                    "Line has already been confirmed by admin and cannot be modified.");
        }
        Instant now = Instant.now();
        line.setCountedQty(countedQty.setScale(QTY_SCALE, RoundingMode.HALF_UP));
    ...
    public StockTakeSessionResponse closeSession(
        ...
        long unconfirmedWithCounts = session.getLines().stream()
                .filter(l -> {
                    // Item has a count submitted if it has countedQty or adminQuantity
                    boolean hasCount = (l.getCountedQty() != null || l.getAdminQuantity() != null);
                    boolean isConfirmed = InventoryConstants.STOCKTAKE_LINE_CONFIRMED.equals(l.getStatus());
    ...
    public StockTakeSessionResponse confirmLine(
        ...
        if (line.getCountedQty() == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Line has no counted quantity");
        }
        Instant now = Instant.now();
        line.setAdminQuantity(adminQuantity != null
        ...
        if (variance.signum() != 0) {
            StockAdjustmentRequest r = persistAdjustmentRequest(businessId, session, line, variance, userId);
    ...
    public ReconciliationResponse getReconciliation(
        ...
        StockTakeSession morning = stockTakeSessionRepository
                ...
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Morning session not found"));
    ...
    private ReconciliationResponse buildReconciliationResponse(
        ...
        for (StockTakeLine line : morning.getLines()) {
            if (InventoryConstants.STOCKTAKE_LINE_CONFIRMED.equals(line.getStatus())) {
                BigDecimal qty = line.getAdminQuantity() != null ? line.getAdminQuantity() : line.getCountedQty();
                if (qty != null) {
        ...
        String morningName = computeSessionName(morning.getSessionType(), morning.getSessionDate());
        String eveningName = computeSessionName(evening.getSessionType(), evening.getSessionDate());
    ...
    private StockAdjustmentRequest persistAdjustmentRequest(
        ...
        BigDecimal countedQty = line.getAdminQuantity() != null
    ...
    private void applyVarianceToStock(
        ...
        if (variance.signum() > 0) {
            BigDecimal unit = resolveInboundUnitCostSafe(businessId, request.getBranchId(),
                    request.getItemId(), unitCostOverride);
    ...
    private StockTakeSessionResponse buildResponse(StockTakeSession session) {
        String name = computeSessionName(session.getSessionType(), session.getSessionDate());

        // Batch-load item names and SKUs in a single query so the frontend
        // never has to do a secondary lookup or page-guess.
        List<String> lineItemIds = session.getLines().stream()
                .map(StockTakeLine::getItemId)
                .distinct()
                .toList();
        Map<String, Item> itemsById = lineItemIds.isEmpty()
                ? Map.of()
                : itemRepository
                        .findByIdInAndBusinessIdAndDeletedAtIsNull(lineItemIds, session.getBusinessId())
                        .stream()
    ...
    private Map<String, BigDecimal> loadOnHandByItem(String businessId, String branchId) {
        List<Object[]> rows = inventoryBatchRepository.sumQuantityRemainingByItemAtBranch(
                businessId, branchId, InventoryConstants.BATCH_STATUS_ACTIVE);
    ...
    private Optional<String> resolveAisleName(String businessId, String itemId) {
        return itemRepository.findByIdAndBusinessIdAndDeletedAtIsNull(itemId, businessId)
                .map(Item::getAisleId)
                .filter(id -> id != null && !id.isBlank())
                .flatMap(aisleId -> aisleRepository.findByIdAndBusinessId(aisleId, businessId))
                .map(Aisle::getName);
    }

    private static String computeSessionName(String sessionType, LocalDate sessionDate) {
        if (sessionType == null || sessionDate == null) {
            return "";
        }
        String capitalized = sessionType.substring(0, 1).toUpperCase() + sessionType.substring(1);
    ...
    private void requireValidSessionType(String sessionType) {
        if (!InventoryConstants.STOCKTAKE_SESSION_TYPE_MORNING.equals(sessionType)
                && !InventoryConstants.STOCKTAKE_SESSION_TYPE_EVENING.equals(sessionType)) {
