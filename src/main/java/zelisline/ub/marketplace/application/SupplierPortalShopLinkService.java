...
public class SupplierPortalShopLinkService {
    ...
    /**
     * Ensure ACTIVE connections for locals already pointing at this marketplace,
     * locals matching contact phone, then mirror shop-linked products into the portal catalogue.
     */
    @Transactional
    public int ensureLinksAndCatalogue(String marketplaceSupplierId) {
        ...
        linked += linkLocalsByPhone(marketplaceSupplierId, marketplace.getContactPhone());
    ...
    private int linkLocalsAlreadyTagged(String marketplaceSupplierId) {
        ...
        for (Supplier local : supplierRepository.findByMarketplaceSupplierIdAndDeletedAtIsNull(
                marketplaceSupplierId)) {
            if (ensureConnection(marketplaceSupplierId, local)) {
                count += 1;
            }
        }
        return count;
    ...
    private int linkLocalsByPhone(String marketplaceSupplierId, String rawPhone) {
        String phone = StkPhoneNormalizer.normalize(rawPhone);
        if (phone == null) {
            phone = SupplierIdentityNormalizer.normalizePhone(rawPhone);
        }
        if (phone == null || phone.isBlank()) {
            return 0;
        }
        String alt = SupplierIdentityNormalizer.normalizePhone(rawPhone);
        if (alt == null) {
            alt = phone;
        }
        ...
        List<SupplierIdentityIndex> indexHits = identityIndexRepository.findTenantByPhoneVariants(
                phone, alt, tail);
        for (SupplierIdentityIndex row : indexHits) {
     ...
     */
    private boolean ensureConnection(String marketplaceSupplierId, Supplier local) {
        ...
        if (connectionRepository.existsByBusinessIdAndMarketplaceSupplierId(
            ...
            // Another local at this shop already linked — do not create a second connection.
            if (local.getMarketplaceSupplierId() == null) {
                local.setMarketplaceSupplierId(marketplaceSupplierId);
                supplierRepository.save(local);
            }
