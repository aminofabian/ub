...
/**
 * In-memory per-IP rate limit for token refresh (abuse guard; legitimate clients
 * single-flight refresh and stay well under the default cap).
 */
@Component
public class RefreshIpRateLimiter {
    ...
    public RefreshIpRateLimiter(
            @Value("${app.security.refresh-rate-limit-per-minute:30}") int maxPerMinute) {
