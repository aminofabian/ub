...
/**
 * Signed tokens for till-access approve/dismiss links (email / WhatsApp / SMS, no login).
 * Format: {@code base64url(requestId).base64url(expEpochSec).base64url(hmac)}.
 */
@Component
public class TillAccessApprovalToken {

    private final byte[] secret;

    public TillAccessApprovalToken(@Value("${app.jwt.secret:}") String jwtSecret) {
        String seed = jwtSecret == null || jwtSecret.isBlank()
                ? "dev-till-access-secret"
                : "till-access:" + jwtSecret;
        this.secret = seed.getBytes(StandardCharsets.UTF_8);
    }

    public String issue(String requestId, Instant expiresAt) {
        String exp = Long.toString(expiresAt.getEpochSecond());
        String payload = encode(requestId) + "." + encode(exp);
        return payload + "." + encode(hmac(payload));
    }

    public String verifyRequestId(String token) {
