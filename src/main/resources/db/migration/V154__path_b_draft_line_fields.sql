-- Path B draft fidelity: grocery qty/cost/sell/expiry on pending lines + client draft JSON.
-- Idempotent: MySQL/MariaDB DDL auto-commits, so a partial apply must be safely re-runnable.
