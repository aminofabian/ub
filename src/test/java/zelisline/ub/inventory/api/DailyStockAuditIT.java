...
class DailyStockAuditIT {
    ...
    void seed() throws Exception {
        ...
        owner = user("owner-da@test", ROLE_OWNER);
        manager = user("mgr-da@test", ROLE_MANAGER);
    ...
    @Test
    void generate_isIdempotentAndIncludesSoldItem() {
        var first =
        ...
        assertThat(first.get().getItems()).extracting("itemId").contains(itemId);
    }
    ...
    @Test
    void review_showsSystemStockForAdmin() throws Exception {
        dailyStockAuditService.generateForBranchIfAbsent(
                TENANT, branchId, auditDate, "system");
        JsonNode session = startMorningSession(manager, ROLE_MANAGER);
        ...
        patchCount(sessionId, lineId, "10", manager, ROLE_MANAGER);

        MvcResult review =
                mockMvc.perform(
                                get("/api/v1/inventory/stock-take/daily-audits/review")
                                        .param("branchId", branchId)
                                        .param("auditDate", auditDate.toString())
                                        .header("X-Tenant-Id", TENANT)
                                        .header(
        ...
        JsonNode body = objectMapper.readTree(review.getResponse().getContentAsString());
        assertThat(body.get("lines").get(0).get("systemStock").decimalValue())
                .isEqualByComparingTo(new BigDecimal("10"));
        assertThat(body.get("lines").get(0).get("morningCount").decimalValue())
                .isEqualByComparingTo(new BigDecimal("10"));
    }

    @Test
    void escalate_appearsInInvestigations() throws Exception {
        dailyStockAuditService.generateForBranchIfAbsent(
                TENANT, branchId, auditDate, "system");
        JsonNode today =
        ...
        String auditId = today.get("auditId").asText();
        JsonNode session = startMorningSession(manager, ROLE_MANAGER);
        patchCount(
                session.get("sessionId").asText(),
                session.get("lines").get(0).get("lineId").asText(),
                "7",
                manager,
                ROLE_MANAGER);
        ...
        mockMvc.perform(
                        post(
                                        "/api/v1/inventory/stock-take/daily-audits/{auditId}/items/{itemId}/escalate",
                                        auditId,
                                        itemId)
                                .contentType(APPLICATION_JSON)
                                .content("{\"notes\":\"Shelf mismatch\"}")
                                .header("X-Tenant-Id", TENANT)
                                .header(TestAuthenticationFilter.HEADER_USER_ID, owner.getId())
                                .header(TestAuthenticationFilter.HEADER_ROLE_ID, ROLE_OWNER))
                .andExpect(status().isOk());

        MvcResult investigations =
                mockMvc.perform(
        ...
        JsonNode rows = objectMapper.readTree(investigations.getResponse().getContentAsString());
        assertThat(rows.isArray()).isTrue();
        assertThat(rows.size()).isGreaterThan(0);
        assertThat(rows.get(0).get("itemId").asText()).isEqualTo(itemId);
    }
    ...
    private void seedSaleYesterday(String soldItemId) {
        ...
        sale.setShiftId("shift-da");
        sale.setStatus(SalesConstants.SALE_STATUS_COMPLETED);
        sale.setIdempotencyKey("idem-da-yesterday");
        ...
        line.setBatchId("dddddddd-dddd-dddd-dddd-dddddddddda1");
    ...
    private void patchCount(
            String sessionId, String lineId, String qty, User user, String roleId)
            throws Exception {
        mockMvc.perform(
                        patch(
                                        "/api/v1/inventory/stock-take/daily-audits/sessions/{sessionId}/lines/{lineId}",
                                        sessionId,
                                        lineId)
                                .contentType(APPLICATION_JSON)
                                .content("{\"countedQty\":" + qty + ",\"note\":\"test\"}")
                                .header("X-Tenant-Id", TENANT)
                                .header(TestAuthenticationFilter.HEADER_USER_ID, user.getId())
                                .header(TestAuthenticationFilter.HEADER_ROLE_ID, roleId))
                .andExpect(status().isOk());
